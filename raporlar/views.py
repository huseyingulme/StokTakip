...
@handle_view_errors(
    error_message="Excel export sırasında bir hata oluştu.",
    redirect_url="raporlar:satis_raporu"
)
@login_required
def satis_raporu_excel(request: Any) -> Any:
    """
    Satış raporu Excel export.
    
    Muhasebe yetkisi gerektirir. Satış faturalarını Excel formatında export eder.
    ...
    try:
        ...
        wb.save(response)
        log_action(request.user, 'export', None, 
                 f'Satış raporu Excel export: {tarih_baslangic} - {tarih_bitis}', request)
        return response
    except Exception as e:
        logger.error(f"Satış raporu Excel export hatası: {str(e)}", exc_info=True)
        raise


@handle_view_errors(
    error_message="Excel export sırasında bir hata oluştu.",
    redirect_url="raporlar:alis_raporu"
)
@login_required
def alis_raporu_excel(request: Any) -> Any:
    """
    Alış raporu Excel export.
    
    Muhasebe yetkisi gerektirir. Alış faturalarını Excel formatında export eder.
    Exception handling ve input validation ile güvenli hale getirilmiştir.
    """
    try:
        # Tarih validation
        ...
        wb.save(response)
        log_action(request.user, 'export', None, 
                 f'Alış raporu Excel export: {tarih_baslangic} - {tarih_bitis}', request)
        return response
    except Exception as e:
        logger.error(f"Alış raporu Excel export hatası: {str(e)}", exc_info=True)
        raise
